...
USERNAME = ''
PASSWORD = ''
DATABASE = 'muic_printing'

myengine = create_engine(f"mssql+pyodbc://{USERNAME}:{quote(PASSWORD)}@{HOST}/{DATABASE}?driver=ODBC+Driver+17+for+SQL+Server")

# ดึงข้อมูลจากฐานข้อมูล
data = pd.read_sql('''select * from Printing_Cleaning order by Year, Num_Month''', myengine)
df = pd.DataFrame(data)

# จัดเรียงข้อมูลตาม Num_Month
df = df.sort_values(['Year', 'Num_Month'])
