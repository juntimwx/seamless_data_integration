...
data = pd.read_excel('../../data/muic_finance/master/Master_GL_2023.05.31.xlsx')
df = pd.DataFrame(data)

# Arrange the columns in the specified order.
df = df[['Id','Description','Group','Group_Description']]
...
try:
    ...
    result = df.to_sql(os.getenv('GL_TABLE'), engine, schema=os.getenv('SCHEMA_MASTER'), index=False, chunksize=500, if_exists='append')
