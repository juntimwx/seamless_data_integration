...
data = pd.read_excel('../../data/muic_asset_inventory/Profile-2025-01-16.xlsx')
df = pd.DataFrame(data,columns=['code','name_thai','surname_thai','name','surname','position_thai','position','department'])
...
try:
    ...
    result = df.to_sql(os.getenv('STAFF_PROFILE_TABLE'), engine, schema=os.getenv('SCHEMA_MASTER'), index=False, chunksize=500, if_exists='append')
